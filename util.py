...
def main():
    ...
    plt.figure()
    plt.subplot(6,1,1)
    plt.plot(t,dynamics.parameters[0]*np.ones((len(t),1)),label='actual value')
    plt.plot(t,ekf.mu[3,:ekf.itr],label='ekf estimate')
    plt.ylabel('k2 []')
    plt.title('Parameters and Estimates vs. Time')

    plt.subplot(6,1,2)
    plt.plot(t,dynamics.parameters[1]*np.ones((len(t),1)))
    plt.plot(t,ekf.mu[4,:ekf.itr])
    plt.ylabel('k0 []')

    plt.subplot(6,1,3)
    plt.plot(t,dynamics.parameters[2]*np.ones((len(t),1)))
    plt.plot(t,ekf.mu[5,:ekf.itr])
    plt.ylabel('k1 []')

    plt.subplot(6,1,4)
    plt.plot(t,dynamics.parameters[3]*np.ones((len(t),1)))
    plt.plot(t,ekf.mu[6,:ekf.itr])
    plt.ylabel('k6 []')

    plt.subplot(6,1,5)
    plt.plot(t,dynamics.parameters[4]*np.ones((len(t),1)))
    plt.plot(t,ekf.mu[7,:ekf.itr])
    plt.ylabel('ka []')

    plt.subplot(6,1,6)
    plt.plot(t,dynamics.parameters[5]*np.ones((len(t),1)))
